function [] = createGlobeFace(uv, steps, R, uk, vk, s0, proj, conts, ub, vb)
    ...
    % Convert the clipped 2d meridian lines into 3d space (transposing the matrices with ')
    [XM3, YM3, ZM3] = to3D(XM', YM', R, uk, vk, ub, vb, XB, YB);
    
    % Convert the clipped 2d parallel lines into 3d space
    [XP3, YP3, ZP3] = to3D(XP', YP', R, uk, vk, ub, vb, XB, YB);
    
    % Draw the 3d meridians in black ('k') with a thin line
    plot3(XM3, YM3, ZM3, 'k', 'LineWidth', 0.2); 
    
    % Draw the 3d parallels in black with a thin line
...
% Loop through all the continents defined in the 'conts' list
    for i=1:length(conts)
        ...
        if polyClipped.NumRegions > 0
            ...
            % Convert these 2d triangle vertices into our 3d dodecahedron space
            [XC3_tri, YC3_tri, ZC3_tri] = to3D(pts(:,1), pts(:,2), R, uk, vk, ub, vb, XB, YB);
            
            % Draw the 3d surface by connecting the 3d vertices according to the triangle mesh list.
            % Turn off the edge color so the triangles seamlessly blend into one solid shape.
            patch('Faces', tr.ConnectivityList, 'Vertices', [XC3_tri, YC3_tri, ZC3_tri], ...
                  'FaceColor', fillColor, 'EdgeColor', 'none', 'FaceAlpha', 0.60);
                  
            % Extract the boundary points (the outline) of the clipped continent
            bnd = polyClipped.Vertices;
            
            % Convert the 2d outline points into 3d space
            [XC3_bnd, YC3_bnd, ZC3_bnd] = to3D(bnd(:,1), bnd(:,2), R, uk, vk, ub, vb, XB, YB);
            
            % Draw the 3d outline using a solid line over the filled shape
